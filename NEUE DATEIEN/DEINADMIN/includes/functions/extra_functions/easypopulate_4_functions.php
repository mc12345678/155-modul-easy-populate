 ...
 * @version $Id: easypopulate_4_functions.php for Zen Cart German 2016-05-22 15:19:14 webchills $
*/
...
function install_easypopulate_4() {
  global $db, $zco_notifier;
	...
	if ( (substr($project,0,5) == "1.3.8") || (substr($project,0,5) == "1.3.9") ) {
		...
		$db->Execute("INSERT INTO ".TABLE_CONFIGURATION_GROUP." (configuration_group_id, language_id, configuration_group_title, configuration_group_description, sort_order, visible) VALUES ('".$group_id."', '43', 'Easy Populate 4', 'Easy Populate 4 Konfiguration', '1', '1')");
      ...
      ('Activate Blank Featured', 'EP4_ACTIVATE_BLANK_FEATURED', 'true', 'On import, if the available date and expiry date are both blank, 0, 0001-01-01, or not imported, should the featured product be made automatically active?<br/> (yes=true or no=false)<br/>(Default: yes)', ".$group_id.", '95', NULL, now(), NULL, 'zen_cfg_select_option(array(\'false\', \'true\'),'),
		...
		");
		$db->Execute("INSERT INTO ".TABLE_CONFIGURATION_LANGUAGE." (configuration_title, configuration_key, configuration_language_id, configuration_description) VALUES
			...
			('Maximal Ausführungszeit','EASYPOPULATE_4_CONFIG_EXECUTION_TIME', '43', 'Wieviele Sekunden hat das Easy Populate Script Zeit, bevor es zu einem Timeout kommt? Diese Einstellung funktioniert nicht auf allen Servern.<br/>(Voreinstellung: 60)'),
			...
			('Benutzerdefinierte Zusatzfelder in der Tabelle products','EASYPOPULATE_4_CONFIG_CUSTOM_FIELDS', '43', 'Wenn Sie die Tabelle products mit individuellen Feldern erweitert haben, dann tragen Siie hier die Namen Ihrer enutzerdefinierte Zusatzfelder ein (mit Komma getrennt, keine Leerzeichen)'),
			('URI von CEON Modul mitexportieren','EASYPOPULATE_4_CONFIG_EXPORT_URI', '43', 'Wenn CEON SEO URI genutzt wird, sollen die URI für Artikel oder Kategorien mitexportiert werden?')
		");
